<template>
  <div class="cont" :class="{loaded: state.isLoggedIn}" v-if="state.oogle">
    <div class="dep-c active"
      ...
      <div class="tot-b" style="margin-top: 100px;">
        ...
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 12px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          ...
          <br>
          <span v-if="this.ogle" class="yld">{{ convertToBnb(parseFloat(this.ogle.value) ) }}</span>
        </div>
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 0px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          <span style="text-transform: capitalize;">Precise</span>
          <br>
          <span v-if="this.ogle" class="yld">{{this.ogle.data.precise}} <abbr v-if="this.amount > 0">(<abbr style="color: #e5c44f;">+1</abbr>)</abbr></span>
        </div>
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 0px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          <span style="text-transform: capitalize;">Boost Hours</span>
          <br>
          <span v-if="this.ogle" class="yld win">{{this.amount ? this.amount * 24 : 0}}</span>
        </div>
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 0px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          <span style="text-transform: capitalize;">Precision Apple</span>
          <br>
      ...
      <div class="bot-h" style="padding-bottom: 340px;">
        ...
        <div class="angl-c">
          ...
          <img :class="{visible: stage == 1}" style="pointer-events: none;opacity:  0;transition: all 0.5s ease 0s;z-index: -1;position: absolute;bottom: 0px;left: 50%;margin-left: -750px;width: 1500px;" src="@/assets/popglow.png">
          <!-- <img :class="{off: stage == 0 || state.game.isLocked}" class="wiz" style="" src="@/assets/smg.svg"> -->
          <investor :flip="false" :class="{off: stage == 0 || state.game.isLocked}"  style="pointer-events: none;position: absolute;bottom: 0px;width: 300px;margin-left: -50px;left: initial;" class="inv" :skin-id="state.user.investor_skin_id"></investor>
          <!-- <img style="z-index: -1;position: absolute;bottom: 0px;right: -95px;margin-right: 0px;width: 400px;" src="@/assets/misc/machine.svg"> -->
          <img style="pointer-events: none;z-index: -1;position: absolute;bottom: 0px;left: -200px;margin-right: 0px;width: 400px;" src="@/assets/misc/shelf.svg">
          <img style="pointer-events: none;z-index: 1;position: absolute;bottom: 0px;left: 75px;margin-right: 0px;width: 220px;" src="@/assets/misc/scales1.svg">
          <item v-if="!this.isSuccessDW" style="pointer-events: none;z-index: 1;position: absolute;bottom: 30px;right: 26px;margin-right: 0px;width: 100px;" :manual="{'path':'basic/item/Apple.png','scale':1.7}"></item>
          <img style="pointer-events: none;z-index: 1;position: absolute;bottom: 0px;right: 25px;margin-right: 0px;width: 100px;" src="@/assets/misc/flask4.svg">
          <img style="pointer-events: none;z-index: -1;position: absolute;bottom: 0px;left: 245px;margin-right: 0px;width: 92px;" src="@/assets/misc/candle2.svg">
          <div class="animated-flame" style="pointer-events: none;" :style="{'background-image': 'url(' + require('@/assets/misc/fire2.svg') + ')'}"></div>
          <!-- <div class="animated-smoke" style="" :style="{'background-image': 'url(' + require('@/assets/misc/smoke1.svg') + ')'}"></div> -->
          <div class="swiper-pagination-line extract" style="">
      ...
      <div class="d-w-b" style="bottom: 180px;position: absolute;" v-if="!(isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError || stage > 0)">
        ...
        <div @click="submitWithdraw" :class="{lockeds: this.notEnough || !this.amount || this.amount < 1}"  style="width: calc(77% - 50px);z-index: 300;overflow: hidden;">
          <span v-if="!this.amount || this.amount < 1">Specify feed amount</span>
          <span v-else-if="this.notEnough">Not enough Precision Apples</span>
          <span v-else>Prepare Eidol lunch</span>
        </div>
      ...
      <button v-if="(isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError || stage > 0)" type="button"
        ...
        :class="{locked: isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError, success: isSuccessDW, slocked: isWaiting || isSending, unlocked: isError}">
          ...
          <span v-else-if="isSending">Processing Feeding...</span>
          <span v-else-if="isEgg && !state.game.isLocked">Cannot feed eggs</span>
          <span v-else-if="!this.ogle">You have no Eidol to feed</span>
          <span v-else-if="extractAmount == 0">You have no Precision Apples</span>
          <span v-else-if="isWaiting">Setting up lunch...</span>
          <span v-else-if="!state.game.isLocked && stage == 1">Approve Feeding</span>
          <span v-else-if="!state.game.isLocked && stage == 0">Prepare Eidol lunch</span>
...
export default {
  ...
  computed: {
    ...
    extractAmount() {
      let found = this.state.items.find(element => element.id == 12);
      if (found) {
    ...
    notEnough() {
      return this.extractAmount - this.amount < 0
    },
    swiper() {
  ...
  methods: {
    ...
    submitWithdraw () {
      ...
      if (this.stage == 0) {
        ...
        .then(() => { 
          axios
            .post(config.api+'/oogles/'+ this.ogle.id +'/apple', {
