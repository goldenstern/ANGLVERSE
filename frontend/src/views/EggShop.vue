  ...
  <div class="cont noselect" :class="{loaded: state.isLoggedIn}">
    ...
    -->
      ...
      <div class="tot-b" style="margin-top: 140px;">
        ...
        <div style="margin-top: 10px;text-align: right;padding-bottom:20px">
          <div style="display: inline-block;font-size: 55px;margin-right: 5px;">⥥
            <span v-if="availableShopItems[selectedIdx]" class="lose">-{{fee.toFixed(8)}}</span>
      ...
      <div class="bot-h" style="padding-bottom: 320px;">
        <div>
          <div v-if="availableShopItems[selectedIdx]" :style="{ bottom: (660 + 0 * ((Math.max(egval/(100),1)-1)/4 + 1)  + 'px!important')}" class="w-i-2" style="z-index: 202;position: absolute;right: 80px;text-align: right;bottom: 538px!important;">
        ...
        <div class="angl-c">
          ...
          <swiper
            style="bottom: 0;position: absolute;width:  calc(100% - 160px)"
            ref="mySwiper"
            :slides-per-view="1"
            :space-between="50"
            @swiper="onSwiper"
          ...
          <img style="z-index: -1;position: absolute;bottom: 0px;left: 0px;margin-left: -530px;width: 920px;" src="@/assets/misc/stand.svg">
          <investor :flip="false"  style="transition:  opacity 1s ease;position: absolute;right: 0px;bottom: 0px;width: 300px;margin-left: -50px;left: initial;transform: scaleX(-1);" class="egg" :skin-id="state.user.investor_skin_id"></investor>
        ...
        <button type="button"
          class="buttdw"
          @click="submit2"
          :class="{success: isSuccessDW, locked: ((isError || state.game.isLocked || isSending || avail) && state.user.start_gas && state.user.start_pack) || !(availableShopItems[selectedIdx] && state.user.balance_ngls), slocked: isSending, unlocked: isError, goldish: ( !state.user.start_gas || !state.user.start_pack ) && !(isError || state.game.isLocked || isSending || !(availableShopItems[selectedIdx] && state.user.balance_ngls)) }">
            ...
            <span v-else-if="state.game.isLocked">Locked for Aeon Computing</span>
            ...
            <span v-else-if="stage == 0 && !state.game.isLocked">Precalculate transaction GAS</span>
            <span v-else-if="stage == 1 && !state.game.isLocked">Approve transaction</span>
        </button>
...
export default {
  ...
  computed: {
    ...
    avail () {
      ...
      } else if (this.availableShopItems[this.selectedIdx] && this.availableShopItems[this.selectedIdx].type == 'egg') {
        return parseFloat(this.availableShopItems[this.selectedIdx].price_ngls) + parseFloat(this.availableShopItems[this.selectedIdx].nodeshare) + parseFloat(this.gas) > parseFloat(state.user.balance_ngls)
  ...
  methods: {
    ...
    loadShopItems () {
      let items = new Array();
      axios.get(`${config.api}/shop`)
        .then(response => {
          for (let item of response.data.data) {
            items.push(item)
          }
          axios.get(`${config.api}/shop?type=item`)
          .then(response => {
            for (let item of response.data.data) {
              items.push(item)
            }
