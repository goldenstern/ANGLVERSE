      ...
      <div class="wall-c active flex-item-2" style="position: relative">
        <div class="tot-b" style="margin-top: 140px;position: fixed;">
          ...
          <div class="wall-h" style="z-index: 202;text-align: right;right: -20px; position: relative;">
            <span class="yld" style="font-size:85px">{{parseFloat(tokenAmount).toFixed(0)}}<img style="width:43px" class="cash" src="@/assets/VectorDAO.png"></span>
          </div>
          <div style="margin-top: 10px;text-align: right;padding-bottom:20px;opacity: 0">
          ...
          <div class="w-i-2" style="margin-top: 12px;z-index: 202;margin-bottom: 0px">
            <span style="text-transform: capitalize;">Social Income<img class="cash" src="@/assets/VectorDAO.png"></span>
            <br>
            <span class="yld win">{{socialIncome}}</span>
          </div>
          <div class="w-i-2" style="margin-top: 0px;z-index: 202;margin-bottom: 0px">
            <span style="text-transform: capitalize;">Patron Income<img class="cash" src="@/assets/VectorDAO.png"></span>
            <br>
            <span class="yld win">{{patronIncome}}</span>
          </div>
          <div class="w-i-2" style="margin-top: 0px;z-index: 202;">
            <span style="text-transform: capitalize;">Invited Patrons</span>
            <br>
            <span class="yld">{{totalPatrons}}</span>
          </div>
        ...
        <div style="width: 100%; text-align: center;position: absolute; bottom: 40px">
          <div style="display: inline-block;">
            <img src="@/assets/misc/ledger.png" style="width: 200px;">
          </div>
        </div>
      </div>
      <div style="width: 100%;" class="flex-item-3 long" :class="{long1: tokenAmount > 0}">
        <div style="color: #e5c44f;font-size: 55px;width: 100%;z-index: 1000;">Monthly gDAO Income</div>
        <div class="height-box" :class="{hidden1: tokenAmount == 0}" style="position: relative;">
          <button @click="drop" style="margin-bottom: 40px" class="button-fixed" :class="{success: isSuccessDW, locked: !isLoaded, slocked: isLoaded, unlocked: isError }">
            <span v-if="!isLoaded">Loading gDAO ledger</span>
        ...
        <div @click="copylink" style="position: relative;">
          ...
          <img src="@/assets/copy.svg" style="width: 60px;z-index: 1000;top: 47px;right: 60px;position: absolute;" @click="copylink" >
        </div>
        <div style="color: #e5c44f;font-size: 55px;width: 100%;z-index: 1000;position: absolute;margin-top: 10px;">Active Patrons: {{patronActive}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    tooMuch () {
      return parseFloat(state.appVars.gdao_chest_supply) < this.tokens * 0.1
    },
    patriciusDuration() {
  ...
  mounted () {
    axios.post(`${config.api}/rooms/gdaoledger`)
    .then(response => {
      this.monthlyIncome = response.data.data.monthly_income;
      this.totalPatrons = response.data.data.total_patrons;
      this.socialIncome = response.data.data.social_income;
      this.patronIncome = response.data.data.patron_income;
      this.burned = response.data.data.burned;
      this.patronActive = response.data.data.patron_active;
      this.isLoaded = true;
    })
    this.currentAnimId = window.requestAnimationFrame(this.onFrame);
