    ...
    <div class="dep-c active"
      ...
      <div class="tot-b" style="margin-top: 100px;">
        ...
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 12px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          ...
          <br>
          <span v-if="this.ogle" class="yld">{{ convertToBnb(parseFloat(this.ogle.value) + (parseFloat(this.amount) ? parseFloat(this.amount) : 0)) }}</span>
        </div>
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 0px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          <span style="text-transform: capitalize;">Feed Amount<img class="cash" src="@/assets/Vector.png"></span>
          <br>
          <span v-if="this.ogle" class="yld win">{{convertToBnb(this.amount ? this.amount : 0)}}</span>
        </div>
        <div v-if="this.ogle" class="w-i-2" style="margin-top: 0px;margin-bottom: 0px;z-index: 202;pointer-events: none;">
          <span style="text-transform: capitalize;">Golden Cookie</span>
          <br>
      ...
      <div class="bot-h" style="padding-bottom: 340px;">
        ...
        <div class="angl-c">
          ...
          <item v-if="!this.isSuccessDW" style="pointer-events: none;z-index: 1;position: absolute;bottom: 30px;right: 26px;margin-right: 0px;width: 100px;" :manual="{'path':'basic/item/Cookie.png','scale':1.7}"></item>
      ...
      <div class="d-w-b" style="bottom: 180px;position: absolute;" v-if="!(isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError || stage > 0)">
        ...
        <div @click="submitWithdraw" :class="{lockeds: this.notEnough || !this.amount || this.amount < 1}"  style="width: calc(77% - 50px);z-index: 300;overflow: hidden;">
          ...
          <span v-else-if="this.notEnough">Not enough Golden Cookies</span>
      ...
      <button v-if="(isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError || stage > 0)" type="button"
        ...
        :class="{locked: isEgg || !this.ogle || extractAmount == 0 || isWaiting || state.game.isLocked || isSending || isError, success: isSuccessDW, slocked: isWaiting || isSending, unlocked: isError}">
          ...
          <span v-else-if="extractAmount == 0">You have no Golden Cookies</span>
...
export default {
  ...
  computed: {
    ...
    extractAmount() {
      let found = this.state.items.find(element => element.id == 6);
      if (found) {
  ...
  methods: {
    ...
    submitWithdraw () {
      ...
      if (this.stage == 0) {
        ...
        .then(() => { 
          axios
            .post(config.api+'/oogles/'+ this.ogle.id +'/feed', {
