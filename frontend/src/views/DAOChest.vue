<template>
  <div class="cont" :class="{loaded: state.isLoggedIn}" v-if="state.user.balance_ngls !== null">
    ...
    <div class="flex-content">
      ...
      <div class="wall-c active flex-item-2" style="position: relative">
        <div class="tot-b" style="margin-top: 140px;position: fixed;">
          ...
          <div class="wall-h" style="z-index: 202;text-align: right;right: -20px; position: relative;">
          ...
          <div style="margin-top: 10px;text-align: right;padding-bottom:20px">
            <div style="display: inline-block;font-size: 55px;margin-right: 5px;">⥣
              <span class="win">{{convertToBnb(tokens * 0.01)}}</span>
            </div><br>
            <span style="font-size: 30px;">Expected Income</span>
          </div>
          <div class="w-i-2" style="margin-top: 12px;z-index: 202;margin-bottom: 0px">
            <span style="text-transform: capitalize;">gDAO Chest Supply<img class="cash" src="@/assets/Vector.png"></span>
            <br>
            <span class="yld">{{state.appVars.gdao_chest_supply ? convertToBnb(state.appVars.gdao_chest_supply - tokens * 0.01) : 0}}</span>
          </div>
          <div class="w-i-2" style="margin-top: 0px;z-index: 202;margin-bottom: 0px">
            <span style="text-transform: capitalize;">Amount to Receive<img class="cash" src="@/assets/Vector.png"></span>
            <br>
            <span class="yld win">{{convertToBnb(tokens * 0.01)}}</span>
          </div>
          <div class="w-i-2" style="margin-top: 0px;z-index: 202;">
            <span style="text-transform: capitalize;">gDAO Claim Fee<img class="cash" src="@/assets/VectorDAO.png"></span>
            <br>
            <span class="yld lose">-{{tokens ? parseInt(tokens) : 0}}</span>
          </div>
          <!-- <p><span style="font-size: 46px;font-family: 'Roboto';">GoldenState Tax Box</span><br>Put in your Tax Commendation to become a Patri for 1 Month<br><br>Patricius title allows you to enter GoldenState Federal Foundation building and receive extra gDAO tokens for important actions made by your Patrons, a Masters who was invited into AnglVerse by your Coop link.<br><br>Patri also able to exchange their gDAO tokens into Angl Shards.</p> -->
        </div>
        <div style="width: 100%; text-align: center;position: absolute; bottom: 40px">
          <div style="display: inline-block;">
            <img v-if="!state.appVars.isTreasureLock" src="@/assets/misc/chest2.png" style="width: 200px;">
            <img v-else src="@/assets/misc/chest1.png" style="width: 200px;">
          </div>
        </div>
      </div>
      <div style="width: 100%;" class="flex-item-3 long" :class="{long1: tokenAmount > 0}">
        <div style="color: #e5c44f;font-size: 55px;width: 100%;z-index: 1000;">gDAO tokens to Burn</div>
        <div style="position: relative;">
          <input onkeypress='return event.charCode >= 48 && event.charCode <= 57' @focus="$event.target.select()" class="reflink" placeholder="Token Amount" v-model="tokens" type="number" />
        </div>
        <div class="height-box" :class="{hidden1: tokenAmount == 0}" style="position: relative;">
          <button @click="drop" style="margin-top: 40px" class="button-fixed" :class="{success: isSuccessDW, locked: isWaiting || state.appVars.isTreasureLock || tokens == 0 || tooMuch || isError || tokenAmount < tokens || isSending, slocked: isWaiting || isSending, unlocked: isError }">
            <span v-if="isError">{{error}}</span>
            <span v-else-if="tokenAmount < tokens">You have not enough gDAO tokens</span>
            <span v-else-if="tooMuch">Not enough gDAO supply</span>
            <span v-else-if="state.appVars.isTreasureLock">gDAO supply is now locked</span>
            <span v-else-if="isSuccessDW">Done! Completing transaction...</span>
            <span v-else-if="tokens == 0">Specify correct token amount</span>
            <span v-else-if="isWaiting">Preparing claim...</span>
            <span v-else-if="!isSending && stage == 1">Claim GoldenState gratuity</span> 
            <span v-else-if="!isSending && stage == 0">Prepare GoldenState gratuity</span> 
            <span v-else-if="isSending">Processing transaction...</span>
          </button>
        </div>
        <div style="color: #e5c44f;font-size: 55px;width: 100%;z-index: 1000;position: absolute;margin-top: 10px;">gDAO tokens: {{tokenAmount}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    isPatri () {
      return ( Date.parse(state.user.patricius) >= Date.now() )   
    },
    tooMuch () {
      return parseFloat(state.appVars.gdao_chest_supply) < this.tokens * 0.01
    },
    patriciusDuration() {
  ...
  methods: {
    ...
    drop () {
      ...
      if (this.stage == 0) {
        ...
        .then(() => {
          axios
            .post(config.api+`/rooms/claimgdao`, {amount: this.tokens})
            .then(() => {
              this.isSuccessDW = true;
