      ...
      <div class="wrap">
        <div class="content">
          ...
          <div class="w-i-2" style="margin-top: 0px;z-index: 202;padding: 0 80px">
            <span>You stand before the Gates of Good, Blessed Gold – ablaze with the golden radiance of perpetual joy and the twilight of greed.
            <br><br>
            Upon the Archway, an inscription reads:
            <br><br>
            <i>We, the Resplendent and Most Glorious Sir Goldenstern, bid thee, O Wanderer, enter our realm.
            <br><br>
            Our universe springs from our desires. Thou dost bear witness to My Light — and the Light is. Thou dost proclaim My Radiance, and thus the spirit of thy dream is proclaimed. 
            <br><br>
            When thou buyest our tokens, be not greedy; lay a generous measure of fine Gold upon the Altar of My Forge (from <b>0.001 BNB GAS deposit</b> <img class="cash" src="@/assets/VectorBnb.png">). And the golden rain of ancient Joy shall not forsake thee for another measure of time and a half. 
            <br><br>
            But if thou layest not a good measure upon My Altar, the universe will forget thee, and thou wilt forget us. Only the warm memory of our ardent love shall remain with thee. 
            <br><br>
            Hail, and welcome!</i>
            <br><br>
            By continuing, thou dost agree to the Covenant and entrust the Magnificent Sir Goldenstern with complete dominion over thy data on all thy devices, until such time as thou withdrawest thy Tokens from the Altar.</span>
            <br><br>
          </div>
          ...
          <!-- <div class="container" style="width: 100%;max-width: initial;padding: 0!important;">
            <label class="option-wrapper" v-for="option in starterPacks" :key="option.id" style="width: 100%;margin-bottom: 40px;"><div v-if="option.id == 4">
                <input @click="errDown" type="radio" name="selectedShopItemId" v-model="selectedShopItemId" :value="option.id" :disabled="(option.price_bnb) > state.user.balance_bnb">
                <div class="option" :class="{'too-expensive': (option.price_bnb) > state.user.balance_bnb}"> 
                  <span class="bub">{{parseFloat(option.price_bnb) | formatCoin}} <img class="cash" src="@/assets/VectorBnb.png"></span>
                  <div v-html="option.description">
                  </div>
                </div>
              </div>
              <div v-else>
                <input @click="errDown" type="radio" name="selectedShopItemId" v-model="selectedShopItemId" :value="option.id" :disabled="parseFloat(option.price_bnb) + parseFloat(gasFee) > state.user.balance_bnb">
                <div class="option" :class="{'too-expensive': parseFloat(option.price_bnb) + parseFloat(gasFee) > state.user.balance_bnb}"> 
                  <span class="bub">{{parseFloat(option.price_bnb) + parseFloat(gasFee) | formatCoin}} <img class="cash" src="@/assets/VectorBnb.png">⥬ {{option.name}}<img class="cash" src="@/assets/Vector.png"></span>
                  <div v-html="option.description">
                  </div>
                </div>
              </div>
...
<!--           <div class="red text" v-show="error">{{error}}</div>
        ...
        <div class="footer">
          <button style="margin-bottom:80px" class="buttdw" type="button" @click="submit" :class="{success: isSuccessDW, locked: isSending || isError || !balance_valid, slocked: isSending, unlocked: isError || !balance_valid}">
            <span v-if="isSuccessDW">Done!</span>
            <span v-else-if="isError">{{error}}</span>
            <span v-else-if="!balance_valid">Insufficent BNB balance</span>
            <span v-else-if="isSending">Processing transaction...</span>
            <span v-else-if="stage == 0">Invoke the Blessing of Sir Goldenstern</span>
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    balance_valid () {
      if ( state.user.balance_bnb < 0.001 ) return false
      return true
    },
    address_valid() {
    ...
    gasFee () {
        if (!this.gasLoaded) return 0.001
        else return this.gasBnb + 0.001
    }
  },
  methods: {
    copywallet () {
    ...
    submit () {
      ...
      axios
        ...
        .then(() => {
          axios
            .post(config.api+`/onboarding/gas`, {})
            .then(() => {
              state.user.start_gas = true;
              state.store();
              this.isSuccessDW = true;
              setTimeout(()=>{utils.update_game();}, 750)
              setTimeout(()=>{this.$router.replace('/starterbuy');}, 1000)
            }).catch(error => {
      ...
      if (this.selectedShopItemId == 4) {
        ...
        if (this.stage == 0) {
          this.isSending = true;
            ...
            .then(() => {
              axios
                .post(config.api+'/wallet/withdraw', {amount: (parseFloat(this.starterPacks[this.selectedShopItemId-1].price_bnb)+0.001), currency: 'bnb', to_address: '0x307d0D271f9Ba42318B55d19EA0A3882D7d4364b', gas: '0'})
                .then((resp) => {
    ...
    superFixed(val) {
      return (Math.floor(val * 100000) / 100000).toFixed(4)
    }
  },
  mounted() {
    const currentUser = state.user
    if (currentUser.start_gas) router.replace('/') 
    this.load();
